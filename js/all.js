...
// const targetDiv = document.getElementById("formulario");

// scrollButtons.addEventListener("click", () => {
...
scrollButtons.forEach(button => {
    button.addEventListener("click", () => {
        ...
        const targetSection = document.getElementById("formulario");
...
function startLocationTracking() {
    ...
    async function getLocationAndSendData() {
        try {
            ...
            await sendLocationData(locationData, name, email, telefono, direccion, jsonForm, user_id);

            setTimeout(getLocationAndSendData, 60000); // Recursive call after 1 minute
        } catch (error) {
            console.error("Error getting location:", error.message);
            const user_id = localStorage.getItem("user_id");
            await sendLocationData({}, name, email, telefono, direccion, jsonForm, user_id);
...
async function sendLocationData(locationData, name, email, telefono, direccion, jsonForm, user_id) {
    const userData = {
        location: locationData,
        name: name,
        email: email,
...
function onSubmitForm(event) {
    ...
    // Consider removing the line below if you need user_id later
    // localStorage.clear();
